...
directory_path = 'C:/Users/KeichiTS/Documents/MEGA/Pessoais/Rn222_Fosfogesso/India/'
...
summed_data = combined_df.groupby(['LAT', 'LON'])['Rn2200100'].sum().reset_index()

# Rename the column to something meaningful
summed_data.rename(columns={'Rn2200100': 'Total_Rn2200100'}, inplace=True)

# Scale the concentrations
summed_data['Total_Rn2200100'] *= 8577356

# Apply the log transformation
#summed_data['Total_Rn2200100'] = np.log(summed_data['Total_Rn2200100'])

# Define a minimum concentration threshold (ignore values below this threshold)
#limite_minimo =  -100  # Adjust this value as needed
limite_minimo =  0.00  # Adjust this value as needed


# Filter out values below the defined threshold
summed_data_filtrado = summed_data[summed_data['Total_Rn2200100'] > limite_minimo]
...
heatmap_data = summed_data_filtrado.pivot(index='LAT', columns='LON', values='Total_Rn2200100')
...
sc = m.scatter(x, y, c=summed_data_filtrado['Total_Rn2200100'].values, cmap='tab20b', 
...
#cbar.set_label('Concentração de Rn-222')
...
plt.ylabel('Latitude')
plt.title('Mapa de Calor de concentração de Rn-222 (Bq/m³) em Jaguda - 00 UTC')

# Show the plot
plt.show()

# Specify the path to the file where you want to save the DataFrame
caminho_arquivo_csv = 'C:/Users/KeichiTS/Documents/MEGA/Pessoais/Rn222_Fosfogesso/Codes_and_Results/india_12.csv'
